...
func waitAndVerifyBinds(c *client.Client, ns string, pvols pvmap, claims pvcmap, testExpected bool) {
	...
	for pvName := range pvols {
		...
		if cr := pv.Spec.ClaimRef; cr != nil && len(cr.Name) > 0 {
			...
			// indicate non-test PVC interference
			pvcKey := makePvcKey(ns, cr.Name)
			_, found := claims[pvcKey]
			Expect(found).To(BeTrue())
...
func completeMultiTest(f *framework.Framework, c *client.Client, ns string, pvols pvmap, claims pvcmap) {
	...
	//? flag if we need to recursive call ourself due to unbound claim remaining in pvcList
	//moreClaims := len(pvcList) > 0 // needed before deleting pvcs since pvcList can be modified
	// 3. handle case of more claims than pvs, where the extra claims are expected
	//    to bind to the newly Available pvs
	//if moreClaims {
	//pvcList = completeMultiTest(f, c, ns, pvPvcBindMap, pvcList)
	//}
}
...
var _ = framework.KubeDescribe("PersistentVolumes", func() {
	...
	Context("with multiple PVs and PVCs", func() {
		...
		// create a pv and pvc variable to be reused in the It blocks
		var pv *api.PersistentVolume
		var pvc *api.PersistentVolumeClaim
		...
		// This It block runs several "sub tests" based on the number of PVs and
		// PVCs created.
		// Note: PVs are created before claims and there are no pre-binding
		//   cases here. Future tests may want to incorporate the following:
		//   a) pre-binding, b) create pvcs before pvs, c) create pvcs and pods
		//   in different namespaces.
		It("should create M PVs and N PVCs: test write access[Flaky]", func() {

			numTests, numPVs, numPVCs := 3, 2, 4

			for cnt := 0; cnt < numTests; cnt++ {
				By(fmt.Sprintf("Creating %v PVs and %v PVCs", numPVs, numPVCs))
				for i := 0; i < numPVs; i++ {
				...
				for i := 0; i < numPVCs; i++ {
					...
					claims[makePvcKey(ns, pvc.Name)] = pvcval{}
				}
